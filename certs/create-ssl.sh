#!/bin/bash
test -f local.know-yourself.com.csr.cnf || cat <<'EOF' >> local.know-yourself.com.csr.cnf
[req]
default_bits = 2048
prompt = no
default_md = sha256
...
test -f local.know-yourself.com.v3.ext || cat <<'EOF' >> local.know-yourself.com.v3.ext
...
test -f self-signed-rootCA.key || openssl genrsa -des3 -out self-signed-rootCA.key 2048
test -f self-signed-rootCA.pem || openssl req -x509 -new -nodes -key self-signed-rootCA.key -sha256 -days 1024 -out self-signed-rootCA.pem -config <( cat local.know-yourself.com.csr.cnf )
